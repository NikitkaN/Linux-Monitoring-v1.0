...
function print_folders_number {
  echo "Total number of folders (including all nested ones) = $(du -h $1 2>/dev/null | wc -l | awk '{print $1}')"
}
...
function print_files_number {
  echo "Total number of files = $(ls -laR $1 2>/dev/null | grep ^- | wc -l | awk '{print $1}')"
}
